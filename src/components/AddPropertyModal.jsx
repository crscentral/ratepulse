...
export default function AddPropertyModal({ onClose, onAdd }) {
  ...
  async function handleSubmit(e) {
    ...
    if (selected) {
      ...
      await onAdd({ name: manualName, location: manualLocation, rooms: Number(rooms) || 0, currency });
    }
    setSubmitting(false);
  }
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6 relative">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <select
            ...
            className="w-full mt-1 mb-4 px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none bg-white"
          >
            {CURRENCIES.map((c) => (
          ...
          <button type="submit" disabled={submitting} className="w-full py-2.5 rounded-md text-sm font-medium text-white bg-navy disabled:opacity-60">
            {submitting ? "Adding property & finding nearby competitors…" : "Add Property"}
          </button>
        </form>
      </div>
    </div>
